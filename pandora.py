...
class Pandora:
    ...
    @staticmethod
    def connect(*, auth_token=None, username=None, password=None):
        ...
        if auth_token:
            pandora.session.headers.update({"X-AuthToken": auth_token})
        else:
    ...
    def _graphql(self, query, **variables):
        return self._request(GRAPH_API_ENDPOINT, {
            "query": query,
            "variables": json.dumps(variables)
        })

    def library_add(self, track_id):
        return self._request(LIBRARY_ADD_ENDPOINT, {
            "request": {
                "pandoraId": track_id
            }})
    ...
    def library_get_items(self, limit=10000, *, cursor=None):
        return self._request(LIBRARY_GET_ENDPOINT, {
            "request": {
                "limit": limit,
                "cursor": cursor
            }})
    ...
    def get_playlist_tracks_info(self, playlist_info, offset=0, limit=100):
        ...
        return self._request(GET_PLAYLIST_TRACKS_ENDPOINT, {
            "request": {
                "pandoraId": playlist_info["pandoraId"],
    ...
    def get_all_playlists(self):
        return self._request(GET_PLAYLISTS_ENDPOINT, {
            "request": {
                "sortOrder": "ALPHA",
                "offset": 0,
                "limit": 100,
    ...
    def playlist_remove(self, playlist_info, track_ids):
        return self._request(PLAYLIST_REMOVE_ENDPOINT, {
            "request": {
                "pandoraId": playlist_info["pandoraId"],
                "trackItemIds": track_ids
            }})
    ...
    def playlist_edit(self, playlist_info, move_set):
        ...
        self._request(EDIT_PLAYLIST_ENDPOINT, {
            "request": {
                "pandoraId": playlist_info["pandoraId"],
                "playlistVersion": playlist_info["version"],
                "moves": moves
            }})
    ...
    def playlist_append(self, playlist_info, item_ids, *, update_info=False):
        ...
        while remaining_item_ids:
            new_playlist_info = self._request(PLAYLIST_APPEND_ENDPOINT, {
                "pandoraId": new_playlist_info["pandoraId"],
                "playlistVersion": new_playlist_info["version"],
                "itemPandoraIds": remaining_item_ids[:MAX_ALBUMS_PER_REQUEST]
            })
    ...
    def playlist_create(self, name):
        return self._request(PLAYLIST_CREATE_ENDPOINT,
                {"details": {"name": name}})

    def search(self, query, types=ALL_SEARCH_TYPES, count=20):
        return self._request(SEARCH_ENDPOINT,
                {"query": query, "types": types, "count": count})
    ...
    def get_artist_discography(self, artist_id, *, annotation_limit=0):
        return self._request(ARTIST_DISCOGRAPHY_ENDPOINT, {
            "artistPandoraId": artist_id,
            "annotationLimit": annotation_limit
        })

    def get_details(self, pandora_id):
        return self._request(GET_DETAILS_ENDPOINT, {
            "pandoraId": pandora_id
        })

    def get_similar_artists(self, artist_id):
        result = self._graphql(GRAPH_API_SIMILAR_ARTISTS_QUERY, pandoraId=artist_id)
        return result["data"]["entity"]["similarArtists"]
